...
const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <div
      ...
      className={`bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between transition-all ${
        isClickable
          ? "cursor-pointer hover:border-primary/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/20"
    ...
    >
      <div className="flex justify-between items-start mb-4 gap-3">
        ...
        <div className="text-right">
          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Live Stats</span>
          {isClickable && (
            <span className="block text-[10px] font-bold text-primary mt-1">Cliquer pour ouvrir</span>
          )}
        </div>
      </div>
      <div>
